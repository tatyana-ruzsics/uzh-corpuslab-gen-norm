...
Usage:
  ...
  [--predictor_weights=WEIGHTS] [--output_format=FORMAT] [--lowercase=LOW] [--morph_vocab=MORPH_VOCAB] [--nmt_type=NMT_TYPE] [--verbose] [--indices=INDICES] [--exclude_eow]
...
Options:
  ...
  --exclude_eow                 whether to include eow symbol for LM scores
"""
...
from norm_soft import SoftDataSet, SoftAttention, load_ed_models

from norm_soft_pos import SoftDataSetFeat
from norm_soft_pos import SoftAttention as SoftAttentionFeat
from norm_soft_pos import load_ed_models as load_ed_models_feat

from norm_soft_char_context import SoftDataSetCont
from norm_soft_char_context import SoftAttention as SoftAttentionCont
from norm_soft_char_context import load_ed_models as load_ed_models_context
...
def predict_syncbeam(input, nmt_models, lm_models, weights, beam = 1, verbose = False, position=None, return_lm_state=False, exclude_eow=False):
        ...
        for m_hypo in m_hypos:
            ...
            while pred_length <= MAX_PRED_SEQ_LEN and len(hypos) > 0: # continue expansion while there is a hypo to expand
                ...
                for s_nmt, s_lm, nmt_log_p, lm_log_p, word, segment in hypos:
                    log_probs = np.array([-dy.log(m.predict_next_(s, position)).npvalue() for m,s in zip(nmt_models,s_nmt)])
#                    print log_probs
...
#        print np.array(weights)
    ...
    if not m_complete_hypos:
        # nothing found
        m_complete_hypos = [(nmt_log_p,lm_log_p, word, nmt_log_p*weights[0] + np.dot(lm_log_p,np.array(weights[1:])),s_lm) for s_nmt, s_lm, nmt_log_p, lm_log_p, word, _ in m_hypos]
            
    m_complete_hypos.sort(key=lambda e: e[2]*weights[0]+np.dot(e[3],np.array(weights[1:])))
    final_hypos = []
    for _,s_lm, nmt_log_p,lm_log_p, word,_,_ in m_complete_hypos[:beam]:
        if return_lm_state:
            final_hypos.append((nmt_log_p,lm_log_p, word, nmt_log_p*weights[0] + np.dot(lm_log_p,np.array(weights[1:])),s_lm))
        else:
            final_hypos.append((nmt_log_p,lm_log_p, word, nmt_log_p*weights[0] + np.dot(lm_log_p,np.array(weights[1:]))))
    return final_hypos

def evaluate_syncbeam_nofeat(data, nmt_models, lm_models, weights, beam, format, verbose=False,exclude_eow=False):
    ...
    for i,(input,output) in enumerate(data):
        ...
        for nmt_model in nmt_models:
            nmt_model.param_init(input)
        predictions = predict_syncbeam(input, nmt_models, lm_models, weights, beam, verbose=verbose, exclude_eow=exclude_eow)
        try:
...
def evaluate_syncbeam_feat(data, nmt_models, lm_models, weights, beam, format, verbose=False,exclude_eow=False):
    ...
    for i,(input,output,feat) in enumerate(data):
        ...
        for lm_model in lm_models:
            lm_model.param_init()
        for nmt_model in nmt_models:
            nmt_model.param_init(input,feat)
        predictions = predict_syncbeam(input, nmt_models, lm_models, weights, beam, verbose=verbose, exclude_eow=exclude_eow)
        try:
            prediction = predictions[0][2]
        except:
...
def evaluate_syncbeam_context(data, nmt_models, lm_models, weights, beam, format, verbose=False,exclude_eow=False):
    ...
    data_len = 0.
    final_results = []
    for i,(inputs,outputs,features,features_in) in enumerate(data):
        dy.renew_cg()
        for nmt_model in nmt_models:
            nmt_model.param_init(inputs)
        if nmt_models[0].hyperparams['POS_FEATURE']:
            if nmt_models[0].hyperparams['AUX_POS_TASK']:
                for nmt_model in nmt_models:
                    nmt_model.param_init_feat_testtime(inputs)
            else:
                for nmt_model in nmt_models:
                    nmt_model.param_init_feat(features_in)
        ...
        for position,(input,output,feature) in enumerate(zip(inputs,outputs,features)):
            ...
            for lm_model in lm_models:
                lm_model.param_init()
            predictions = predict_syncbeam(input, nmt_models, lm_models, weights, beam, verbose=verbose, position=position, exclude_eow=exclude_eow)
            try:
                prediction = predictions[0][2]
            except:
                print i, input, output, predictions
...
if __name__ == "__main__":
    ...
    nmt_type  = arguments['--nmt_type']
    if nmt_type =='norm_soft_pos':
        data_set = SoftDataSetFeat
        evaluate_syncbeam = evaluate_syncbeam_feat
        model_loader = load_ed_models_feat
    elif nmt_type.startswith('norm_soft_char_context'):
        data_set = SoftDataSetCont
        evaluate_syncbeam = evaluate_syncbeam_context
        model_loader = load_ed_models_context
    else:
        data_set = SoftDataSet
        evaluate_syncbeam = evaluate_syncbeam_nofeat
        model_loader = load_ed_models
    ...
    nmt_models = model_loader(arguments['ED_MODEL_FOLDER'].split(','),pc)
    ensemble_number = len(nmt_models)
