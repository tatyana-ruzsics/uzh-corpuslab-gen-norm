...
def evaluate_baseline(trainin,gold,input_format,lowercase=False, file_out_errors=None, conll_format=False):
    ...
    for i,line in enumerate(gold_f):
        ...
        if len(line.strip()) != 0:
            ...
            if (not conll_format) or (conll_format and not line.startswith('#') and not(lineitems[pos_col].lower()=='punct' or any(c in '@+._/1234567890' for c in lineitems[pred]+lineitems[input])) ):
                ...
                if not word in test_seg_dict.keys():
                    test_seg_dict[word] = {}
                    test_seg_dict[word][segm]=1
                else:
                    if segm not in test_seg_dict[word].keys():
                        test_seg_dict[word][segm]=1
                    else:
                        test_seg_dict[word][segm]+=1
...
#            w, w_segm = line.split('\t')
    ...
    not_amb_segm_test = {k:v for k,v in test_seg_dict.items() if k not in amb_segm_test.keys()}
    seen_freq = {k:sum(v.values()) for k,v in not_amb_segm_test.items() if k in train_lexicon_w.keys()}
    seen = sum(seen_freq.values())
    corr_seen_b = 0 # number of correct seen words
    ...
    unseen_m_freq = {k:sum(v.values()) for k,v in not_amb_segm_test.items() if ( not k in train_lexicon_w.keys() and not all(m in train_lexicon_m.keys() for m in v.keys()[0].split(' ')) )}
    unseen_m = sum(unseen_m_freq.values())
    corr_unseen_m_b = 0 # number of correct unseen words - new morphs
    
    unseen_new_comb = unseen - unseen_m
    corr_unseen_comb_b = 0 # number of correct unseen words - new combinations
...
#                        print w, train_dict[w], test_dict[w], pos
    ...
    for i,line in enumerate(gold_f):
        ...
        if len(line.strip()) !=0:
            ...
            if (not conll_format) or (conll_format and not line.startswith('#') and not(lineitems[pos_col].lower()=='punct' or any(c in '@+._/1234567890' for c in lineitems[pred]+lineitems[input])) ):
                ...
                if predict:
                    # system predictions - ambiguity evaluation
    ...
    if predict:
        ...
        if seen !=0:
            print "{:>60} {:11d} {:8.2f}%\n".format("- unique:", corr_seen_b, corr_seen_b/seen*100)
        print "{:>60} {:11d} {:8.2f}%\n".format("- new:", corr_unseen_b, corr_unseen_b/unseen*100)
